...
## load data
dia_lsmeans <- read.xlsx("C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/ls_means_plot.xlsx",
                         sheetIndex = 1)
sca_lsmeans <- read.xlsx("C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/ls_means_plot.xlsx",
                         sheetIndex = 2)
...
dia_cl_pl <- ggplot(dia_lsmeans %>% subset(., !is.na(.$climate)), 
  ...
  ylab("Intact seeds per capsule") + scale_color_manual(values = myPalette) + 
...
sca_sea <- ggplot(sca_lsmeans, aes(x = Season, y = estimate)) + geom_point() + theme_bw() + 
  ...
  ylim(10, 40) + xlab("") + ylab("Intact seeds per head") + 
...
dia_sea <- ggplot(dia_lsmeans %>% subset(., !is.na(.$Season)), aes(x = Season, y = estimate)) + geom_point() + theme_bw() + 
  geom_errorbar(aes(ymin = estimate - Standard.error, ymax = estimate + Standard.error), width = 0.25) + 
  ylim(10, 40) + xlab("") + ylab("Intact seeds per capsule") + 
...
ggsave(dia_cl_pl, 
       filename = "C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/dia_pl_cl.tiff",
       dpi = 800)
ggsave(dia_sca_sea, 
       filename = "C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/dia_sca_sea.tiff",
       dpi = 800)

## appendix figures
sca_lsmeans_bagged <- read.xlsx("C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/ls_means_plot.xlsx",
                                sheetIndex = 3)
dia_lsmeans_bagged <- read.xlsx("C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/ls_means_plot.xlsx",
                                sheetIndex = 4)
...
sca_bag_pl <- ggplot(sca_lsmeans_bagged %>% subset(., !is.na(.$pollination)), 
  ...
  ylim(0,40) + labs(title = expression(italic(paste("S. ochroleuca"))),
                    y = "Intact seeds per seed head", x = "") + 
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())+
  geom_text(x = 0.65, y = 40, label = "b.)", size = 5)
...
dia_bag_pl <- ggplot(dia_lsmeans_bagged %>% subset(., !is.na(.$pollination)), 
  ...
  ylim(0,40) + labs(title = expression(italic(paste("D. carthusianorum"))),
                    y = "Intact seeds per seed capsule", x = "") + 
  geom_text(x = 0.65, y = 40, label = "a.)", size = 5)
...
ggsave(dia_sca_bagged_pl, 
       filename = "C://Users/ma22buky/Documents/01_PhD/01_Pollination_experiment/Results/SAS_results/dia_sca_bagged.tiff", 
       dpi = 800)
